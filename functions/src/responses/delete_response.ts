...
export const deleteResponse = onCall(async (request) => {
    ...
    if (!queryId || !responseId) {
        throw new HttpsError("invalid-argument", "Query ID and Response ID are required.");
    }
    ...
    if (responseSnap.data()?.postedBy?.uid !== uid) {
        throw new HttpsError("permission-denied", "You can only delete your own responses.");
    }
